import Foundation

/// 시스템 언어로 문구를 찾는다. 앱의 언어 설정을 따르지 않는다.
///
/// 도커 우클릭 메뉴에서 Dock이 붙이는 항목(옵션·Finder에서 보기·종료)은 항상
/// 시스템 언어이고 앱이 바꿀 수 없다. 우리 항목까지 앱 언어를 따르면 한 메뉴에
/// 두 언어가 섞이므로, 이 메뉴만 시스템 언어에 맞춘다. 팝오버와 설정 창은
/// 그대로 사용자가 고른 언어를 쓴다.
enum SystemLanguage {
    /// `L()`이 쓰는 번들과 다르다. 언어 설정이 앱 도메인의 `AppleLanguages`로
    /// 구현돼 `Bundle.main`도 그걸 따르므로, 전역 도메인을 직접 읽는다.
    private static let bundle: Bundle = {
        // UserDefaults(suiteName: "NSGlobalDomain")은 nil을 주고 Bundle.main으로
        // 떨어져 앱 언어를 따라간다. CFPreferences로 읽어야 한다.
        let preferred = CFPreferencesCopyValue(
            "AppleLanguages" as CFString, kCFPreferencesAnyApplication,
            kCFPreferencesCurrentUser, kCFPreferencesAnyHost) as? [String] ?? []
        let available = Bundle.main.localizations
        for language in preferred {
            let code = String(language.prefix(2))
            guard let match = available.first(where: { $0.hasPrefix(code) }),
                  let path = Bundle.main.path(forResource: match, ofType: "lproj"),
                  let found = Bundle(path: path) else { continue }
            return found
        }
        return .main
    }()

    /// 번역이 없으면 키(= 한국어 원문)를 그대로 돌려준다.
    static func text(_ key: String) -> String {
        bundle.localizedString(forKey: key, value: key, table: nil)
    }
}
