...
/// 정리 후보 한 줄. 판정이 휴리스틱이라 근거를 부제로 그대로 노출해 사용자가 검산하게 한다.
struct ResidueRow: View {
    ...
    /// 종료는 되돌릴 수 없어(저장 안 된 작업이 날아간다) 한 번 더 물어본다.
    /// alert 창은 팝오버를 닫아버릴 수 있어 같은 자리에서 확인을 받는다.
...
///
/// 여섯 개를 다 늘어놓으면 두 줄을 먹고 결정적인 근거가 묻힌다. 중요한 순서로
/// 정렬해 세 개만 보여주고 전체는 툴팁에 남긴다.
enum Evidence {
