...
/// 지금 쓰이는 중인 후보를 가려낸다. 살아 있는 프로세스가 물고 있는 캐시나
/// node_modules를 옮기면 휴지통을 비울 때 "사용 중" 경고에 막히고, 앱이 열린 핸들로
/// 휴지통 안의 폴더에 계속 써서 옮긴 캐시가 거기서 다시 자란다.
///
/// 불리언이 아니라 누가 쓰는지 이름을 돌려준다. 무엇을 꺼야 할지는 사용자가 이름을
/// 보고 정한다.
///
/// 판정은 프로세스 메타데이터(실행 경로·작업 디렉터리)로만 한다. lsof는 node_modules
/// 하나에도 수만 파일이라 스캔마다 돌리기엔 너무 느리다. 그래서 판정이 완전하지
/// 않지만, 규칙이 좁아 멀쩡한 후보를 숨기는 쪽으로는 틀리지 않는다.
public struct ReclaimInUse: Sendable {
    /// 판정에 필요한 만큼만 남긴 프로세스 한 줄.
    struct Proc: Sendable {
        let name: String        // 앱 이름 또는 실행 파일 이름
        let cwd: String?
        let execPath: String
    }
    ...
    public init(samples: [ProcessSample]) {
        ...
        for sample in samples {
            ...
            // 누가 쓰는지 말할 수 없으면 후보를 숨기지 않는 쪽이 맞다.
            let name = appName
    ...
    ///
    /// 실행 중 여부가 의미 있는 종류에만 반응한다. 빌드 캐시는 Xcode가 떠 있어도
    ...
    /// 캐시 폴더 이름이 가리키는 실행 중인 앱. 폴더 이름은 앱 이름일 수도 번들ID일
    /// 수도 있어 양쪽에 다 물어본다.
    ...
    /// 실행 파일 경로에서 .app 번들 경로를 뽑는다. 번들 밖 실행 파일은 nil이다.
