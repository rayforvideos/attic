...
/// 로그인·부팅할 때 함께 올라오는 항목 하나. **읽기 전용**이다.
public struct StartupItem: Sendable, Equatable, Identifiable {
    public enum Domain: Sendable, Equatable {
        ...
        case user
        /// `/Library/LaunchAgents`, `/Library/LaunchDaemons` — 관리자 권한이
        /// 있어야 바꿀 수 있어 여기서는 보여주기만 한다.
        case system
    }

    /// 등록만 남고 실체가 없는 경우. 이 둘을 뭉뚱그리면 안 된다 — 화면에
    /// "프로그램이 없어요"라고 말했는데 실은 빈 파일이면 거짓말이 된다.
...
/// 자동 실행 항목을 **훑어보기만** 한다. 끄고 켜는 것은 LaunchAgentManager가
/// 사용자 도메인에서만 한다 — 그 안전 경계를 넓히지 않으려고 타입을 나눴다.
///
/// 왜 시스템 도메인까지 보나: 일반 사용자의 맥에서 부팅할 때 조용히 올라오는
/// 것들이 대부분 여기 있다(백신, 은행 보안 프로그램, 각종 업데이터). 끄지는
/// 못해도 **무엇이 올라오는지 아는 것**과 **지워진 앱의 찌꺼기를 찾아내는 것**은
/// 사용자가 할 수 있는 일로 이어진다(그 앱을 지우거나, 제대로 삭제하거나).
public struct StartupInventory: Sendable {
    ...
    /// plist 하나를 읽어 상태를 판정한다. 읽을 수 없으면 nil — **모르는 것을
    /// "찌꺼기"라고 말하지 않는다.** (실제로 권한·심볼릭 링크 때문에 못 읽는
    /// 파일이 있었고, 그걸 고아로 잘못 셌었다.)
